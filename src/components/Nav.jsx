...
import { FaSearch, FaTimes, FaAngleDown, FaUserCircle, FaFilm, FaTv } from 'react-icons/fa';
import { User, Settings, LogOut, Home, Search, Star, UserCircle, Menu } from 'lucide-react';
...
export default function Nav() {
  ...
  const handleRecommendationClick = (item) => {
    if (item.media_type === 'user') router.push(`/user/${item.name}`);
    else if (item.media_type === 'movie') router.push(`/movie/${item.id}`);
  ...
  return (
    <>
        ...
        <div className="max-w-7xl xl:mx-40 mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link href="/" className="text-xl font-bold text-primary flex items-center">
              <Image src="/wz0.png" alt="wzone" width={60} height={20} className="h-auto transition-transform duration-500 group-hover:scale-110" priority={true} loader={cloudinaryLoader} />
            </Link>
                  ...
                  {recommendations.map(item => (
                    <div key={item.id} className="flex items-center gap-3 px-4 py-2 hover:bg-secondary cursor-pointer text-sm" onClick={() => handleRecommendationClick(item)}>
                      {item.media_type === 'user' ? (
                        item.avatarUrl ? <Image src={item.avatarUrl} alt={item.name} width={24} height={24} className="rounded-full" loader={cloudinaryLoader} /> : <FaUserCircle className="w-6 h-6 text-foreground/50" />
                      ) : (
                        item.poster_path ? <Image src={`https://image.tmdb.org/t/p/w92${item.poster_path}`} alt={item.name} width={24} height={36} className="rounded-sm" loader={cloudinaryLoader} /> : (item.media_type === 'movie' ? <FaFilm className="w-6 h-6 text-foreground/50" /> : <FaTv className="w-6 h-6 text-foreground/50" />)
                      )}
                      <span>{item.name}</span>
                    </div>
                  ))}
                  ...
                  <>
                    ...
                    <div className="relative" ref={profileRef}>
                      <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white font-bold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-background focus:ring-primary">
                        {user.avatarUrl ? <Image src={user.avatarUrl} alt="User Avatar" width={36} height={36} className="w-full h-full rounded-full object-cover" loader={cloudinaryLoader} /> : getAvatarInitial()}
                      </button>
                      {isProfileOpen && (
                        <div className="absolute right-0 mt-2 w-56 bg-surface rounded-md shadow-lg py-1 ring-1 ring-black ring-opacity-5">
                          ...
                          {user.username && (
                            <Link href={`/user/${user.username}`} onClick={() => setIsProfileOpen(false)} className="flex items-center w-full text-left gap-3 px-4 py-2 text-sm text-foreground hover:bg-secondary">
                              <User className="h-4 w-4" /><span>View Profile</span>
                            </Link>
                          )}
      ...
      {/* Mobile Bottom Nav */}
      <nav
        className="md:hidden fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur-lg z-20 flex justify-around items-center px-4"
        style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)', height: 'calc(56px + env(safe-area-inset-bottom, 0px))', boxShadow: '0 -1px 12px rgba(0,0,0,0.35)' }}
      >
        <Link href="/" className="flex flex-col items-center justify-center gap-0.5 text-foreground/50 hover:text-primary transition-colors py-2 px-3">
          <Home size={20} strokeWidth={1.8} />
          <span className="text-[10px] font-medium">Home</span>
        </Link>
        <Link href="/search" className="flex flex-col items-center justify-center gap-0.5 text-foreground/50 hover:text-primary transition-colors py-2 px-3">
          <Search size={20} strokeWidth={1.8} />
          <span className="text-[10px] font-medium">Search</span>
        </Link>
        <Link href="/top-rated/movies" className="flex flex-col items-center justify-center gap-0.5 text-foreground/50 hover:text-primary transition-colors py-2 px-3">
          <Star size={20} strokeWidth={1.8} />
          <span className="text-[10px] font-medium">Top Rated</span>
        </Link>
        <Link href={profileLink} className="flex flex-col items-center justify-center gap-0.5 text-foreground/50 hover:text-primary transition-colors py-2 px-3">
          <UserCircle size={20} strokeWidth={1.8} />
          <span className="text-[10px] font-medium">{user ? "Profile" : "Sign In"}</span>
        </Link>
        <button onClick={() => setIsMenuOpen(true)} className="flex flex-col items-center justify-center gap-0.5 text-foreground/50 hover:text-primary transition-colors py-2 px-3">
      ...
      <div className={`fixed top-0 left-0 w-full h-screen bg-surface z-50 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:hidden`}>
        <div className="flex justify-between items-center p-4 border-b border-secondary">
          <Link href="/" onClick={closeMenu} className="text-2xl font-bold text-primary">watchzone</Link>
